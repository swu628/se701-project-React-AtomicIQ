...
export default function Wordle({allowedAttempts, properties, availableElements}: WordleProps) {
    ...
    const getCellContent = (rowIndex: number, colIndex: number) => {
        ...
        if (checkCellCorrectness(rowIndex, colIndex)) {
            switch (properties[colIndex - 1]) {
                    ...
                    return (<AvatarGroup max={2}>
                        {Array.from(elementData.origins, (origin) => (
                            <Tooltip title={origin} arrow>
                                <Avatar alt={origin} src={`src/assets/origin/${origin}.png`} variant="square"/>
                            </Tooltip>
                        ))}
                    </AvatarGroup>);
                case ElementProperty.Countries:
                    return (<AvatarGroup max={2}>
                        {Array.from(elementData.countries, (country) => (
                            <Tooltip title={country} arrow>
                                <Avatar alt={country} src={`src/assets/flags/${country}.gif`} variant="square"/>
                            </Tooltip>
                        ))}
                    </AvatarGroup>);
                case ElementProperty.Color:
                ...
                case ElementProperty.NaturalPhase:
                    return (<Tooltip title={elementData.naturalPhase} arrow>
                        <Avatar alt={elementData.naturalPhase} src={`src/assets/phase/${elementData.naturalPhase}.gif`}/>
                    </Tooltip>);
                default:
                    return "";
            }
            ...
            switch (properties[colIndex - 1]) {
                ...
                case ElementProperty.Period:
                    if (elementData.period > answer.period) {
                        return <Tooltip title="Go up, too low in the periodic table"
                                        arrow><span>{elementData.period} ↑</span></Tooltip>;
                    } else {
                        return <Tooltip title="Go down, too high in the periodic table"
                                        arrow><span>{elementData.period} ↓</span></Tooltip>;
                    }
                case ElementProperty.Group:
                    if (elementData.group > answer.group) {
                        return <Tooltip title="Go left, too right in the periodic table"
                                        arrow><span>{elementData.group} ←</span></Tooltip>;
                    } else {
                        return <Tooltip title="Go right, too left in the periodic table"
                                        arrow><span>{elementData.group} →</span></Tooltip>;
                    }
                case ElementProperty.Origins:
                    return (<AvatarGroup max={2}>
        ...
        Array.from({length: cols + 1}, (_, colIndex) => {
            if (colIndex === 0 && rowIndex === 0) {
                ...
                return (
                    ...
                    >
                        <Tooltip title={'Click to see more about ' + guessedElementData[rowIndex - 1]?.name} arrow>
                ...
                return (
                    <div
                        key={`${rowIndex}-${colIndex}`}
                        className={`flex justify-center items-center text-center w-24 h-24 border cursor-pointer ` + (getCellColor(rowIndex, colIndex))}
                    >
    ...
    const elementButtons = Array.from(Object.values(Element), (element) => (
        <Button disabled={guesses.includes(element) || !availableElements.includes(element)} onClick={guess(element)} key={element}>
            {element}
        </Button>
    ));
